...
serve(async (req) => {
  ...
  try {
    ...
    const tokenRes = await fetch(
      `${dbUrl}/users/${target_user_id}/fcmToken.json?access_token=${accessToken}`,
    );
    const fcmToken = await tokenRes.json();

    if (typeof fcmToken !== 'string' || !fcmToken) {
      const status = fcmToken === null ? 404 : 400;
      const error = fcmToken === null
        ? 'Target user has no FCM token'
        : `fcmToken at /users/${target_user_id}/fcmToken is not a string (got ${typeof fcmToken})`;
